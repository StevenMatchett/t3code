...
| `UP-005` | Shell execution | `packages/contracts/src/terminal.ts`, `packages/contracts/src/rpc.ts`, `apps/server/src/ws.ts`, `apps/server/src/auth/RpcAuthorization.ts`, `apps/server/src/terminal/executeShellCommand.ts` | Execute non-interactive commands in the connected environment and return bounded output for TUI shell messages. | Yes | N/A | Medium | Upstream provides an equivalent command execution RPC. |
